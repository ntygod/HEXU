...
- W1 is dark-first with cyan primary actions, a complete light theme, compact/comfortable density, a collapsible navigation rail/context guide, task-centered process/code panels, stable prompt controls and contextual drawers. Use the semantic values in `docs/design/tokens.css`; these reference tokens are not yet imported by the application. Migrate existing shared components in small slices, not via scattered page palettes or an unrequested UI rewrite.
