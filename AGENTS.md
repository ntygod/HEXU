...
- W1 is dark-first with cyan primary actions, a complete light theme, compact/comfortable density, a collapsible navigation rail/context guide, task-centered process/code panels, stable prompt controls and contextual drawers. Runtime values live in `packages/ui/src/tokens.css`; `docs/design/tokens.css` forwards to that single source. Shared controls and the W1-01/02 shell now consume them; remaining page layouts still need their planned slices. Preserve existing explicit theme preferences and keep guide preferences scoped by identity/space. Do not introduce a second page palette or an unrequested UI rewrite.
