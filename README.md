...
> **当前阶段：E1a 本机原生文件工具接入。** 已有真实项目、任务、讨论、成果持久化，并新增可选 Claude Code CLI 调用、显式授权 Git 目录、结构化输出、目录互斥和进程组停止。默认仍是无需密钥的模拟预览。**原生路径尚未使用真实提供方账户联调；当前测试使用明确标识的协议替身，不能视为真实模型兼容性证明。** 尚无多用户认证、Codex 原生接入或远程节点，不可作为公网团队服务。
...
| 本机 Claude Code：只读分析或文件修改 | **实验性原生适配**；CLI 进程和 Git 已实现，提供方账户联调待完成 |
| 显式目录授权、实际 Git 变更、结果输出与停止 | 已实现；测试通过协议替身执行真实本地文件操作 |
| 原生会话恢复／运行中追加要求／Bash／MCP | 尚未接入；新执行使用任务说明和最近工作记录 |
| Codex 原生、多成员登录、独立 Runner 和远程执行 | 尚未实现 |
| 示例订单预览 | 虚构业务示例，不是通用预览隧道 |

模拟名称不代表原生工具已接通。设置页区分本机 Claude Code 的能力探测、密钥是否配置与 Codex 待接入状态；工具检测可用不代表凭证有效或真实模型已经测试。
...
E1a 不开放 Bash、网络工具、MCP、仓库 Hooks，不能替你运行安装、构建或测试命令。CLI 调用模型本身仍需联网；文件工具约束不是操作系统沙箱。Linux 已做工程测试，macOS 的 POSIX 路径尚未经平台实测；Windows 原生执行暂不可用。详见 [原生使用说明](docs/engineering/native-execution.md)。
...
packages/adapters/claude-code  原生 JSONL 协议与受限文件工具配置
