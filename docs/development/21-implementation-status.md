...
[E2a 完整开发检查 36213550695](https://github.com/ntygod/HEXU/actions/runs/36213550695) 已在 GitHub-hosted Ubuntu / Node 24 通过。工作流持久化后实际检查源码为 `6dd28eaf24a30c678f0d76d8bd2ef8d8b1a1da5f`，不是触发工作流的临时传输提交。

| 检查 | 实际结果 |
| --- | --- |
| 锁文件全新安装、格式检查 | 通过 |
| 双端 TypeScript 与前后端构建 | 通过 |
| 领域、存储、API、进程、Git、认证与权限测试 | 92 条通过，无跳过 |
| Chromium 浏览器流程 | 20 条通过，0 失败、0 不稳定、0 跳过 |
| 浏览器产物 | 已下载并读取报告；已抽查项目成员权限和手机深色页面截图 |

新增的 3 条浏览器流程实际完成首次建立账号、第二账号通过邀请加入、私有任务隔离、项目只读转编辑、跨账号讨论、项目与空间撤权、会话撤销及重新登录。既有 17 条流程保留。测试过程中修正了密码和项目成员选择框的可访问名称，没有删除失败用例或增加跳过。

本地 Chromium 被环境策略阻止访问 localhost（ERR_BLOCKED_BY_ADMINISTRATOR），不计为本地浏览器通过；上述浏览器结果来自仓库 CI。截图不替代功能操作和服务端断言。主分支最终状态仍以对应提交的只读 CI 记录为准。
