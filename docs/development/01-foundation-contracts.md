...
建立共享界面、控制服务和执行器的工程骨架。客户端形态已由 [ADR-0008](../engineering/adr-0008-client-surfaces.md) 确定为桌面优先、可选团队服务与 Web 协作入口；现有 Web + Runner 路径继续复用。尚未接通真实工具时，模拟适配器产生明确标识的状态，不能混入项目实际记录。

## 2. 可执行的建议基线

当前采用 TypeScript monorepo、React + Vite 共享界面、Fastify 控制服务、Node/TypeScript 执行器、分模式 SQLite、Node 24 与 npm。版本以根 package.json 和 package-lock.json 为准；不再按初始草案切换 pnpm。正式 PostgreSQL 属于后续交付，桌面框架尚未选定，不因新增宿主重写 Runner。

已有基线与未决选型分开记录，升级时核对兼容性，不在 CI 中浮动安装 latest。认证使用经 IdentityPort 隔离的 Better Auth；正式部署与恢复能力仍按实际状态交付。依据见 20 和实现进度。
...
迁移文件按包分段，由单一迁移历史执行。新增字段必须考虑已有 SQLite 数据、原 ID 与 preview／team 隔离，不设计第二套 v1.0 验收模型。连接、事务和迁移已有实现，附件与正式存储按剩余工作项推进；业务表在归属计划中维护。
