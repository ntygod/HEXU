...
本轮推进 **04-01** 项目任务真人负责人改派，联动 **04-03/06、07、10、11**：修订式选择、不可变历史、创建者/执行发起者分别记录、待接续原子暂停及 W1 入口。03-06 归档/恢复已合入，双工具有界恢复路径保留，真实模型仍未联调。下一步为参与者关系与共用筛选，见 [22](22-next-delivery.md)。
...
| HX-DEV-04-01 | Task 基础、归属和修订 | BE | 部分实现 | Task 修订、真实个人/项目访问、项目任务真人改派与不可变历史已接入；新记录分别保存创建者/执行发起者，旧身份未知不倒填；参与者、私有转共享及正式存储完整模型仍缺。 | [任务](../../packages/db/src/store.ts) / [改派](../../packages/db/src/task-assignment.ts) |
...
| HX-DEV-04-03 | 列表、看板、排序、筛选与等待原因 | FE/BE | 部分实现 | W1 项目列表、看板和任务列表共用真实 Task 状态，保留标题筛选和轻量新建；持久排序、完整多条件筛选仍未交付。 | [项目](../../apps/web/src/projects.tsx) |
...
| HX-DEV-04-06 | 项目/个人入口与 Task DTO 统一 | BE/FE | 部分实现 | 真实项目/个人 Task 共用 DTO、创建和权限；项目可见任务的负责人修订式改派保留创建者和执行身份；跨空间/私有选择性转移与完整归属切换未完成。 | [任务](../../packages/db/src/store.ts) |
...
| HX-DEV-15-02 | 工作台、待回复与成果视图 | FE | 部分实现 | W1 工作台从当前可见 Task/Run/Result 聚合个人／团队工作、关注事项和最近成果，已完成任务的活动执行仍展示；完整通知、协助与接手提醒未交付。 | [工作台](../../apps/web/src/workbench.tsx) |
| HX-DEV-15-03 | 通知投影、去重与偏好 | BE/FE | 未实现 | 通知投影、去重和个人偏好未实现。 | —（尚无可用实现） |
| HX-DEV-15-04 | 有权限的中文关键词搜索 | BE/FE | 部分实现 | W1 命令入口复用实际权限过滤的标题／编号搜索；请求取消与错误清空旧结果已有。资料／消息全文、语义搜索及完整检索分页未交付。 | [搜索](../../apps/web/src/command-menu.tsx) |
...
03-06 本机归档切片：在既有项目设置上增加状态修订、执行入口和许可校验、等待 Operation 持久暂停与可选择停止；恢复项目不重启工作。原 102 项仍 2 完成 / 61 部分 / 39 未实现，不将本机能力算作跨电脑产品交付。
