...
本轮推进 **04-01/03/06** 参与关系与人员筛选：独立修订/历史、本人加入退出、撤权结束关系、看板/列表/API 共用交集与 URL 条件。已交付的真人改派、03-06 归档和双工具有界恢复路径保留，真实模型仍未联调。下一步为项目资料与上下文，见 [22](22-next-delivery.md)。
...
| HX-DEV-04-01 | Task 基础、归属和修订 | BE | 部分实现 | Task 修订、真实权限、真人改派/历史和独立参与关系已有；本人加入退出、编辑者管理、撤权结束和历史回执不复活已接入。参与不授予权限或改动运行材料；私有转共享及正式存储完整模型仍缺。 | [任务](../../packages/db/src/store.ts) / [改派](../../packages/db/src/task-assignment.ts) / [参与者](../../packages/db/src/task-participants.ts) |
