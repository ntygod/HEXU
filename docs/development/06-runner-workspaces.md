...
独立 CLI 的本人执行策略、真实进程、持久命令/事件日志与 preview 共享目录占用已接入。根目录和父子目录重叠均阻止同时启动；未知写入者保留占用，原生恢复不自动重跑。当前一节点一个执行，只有本机明确配置的工具/目录/模式可用。WSS、跨电脑、Windows、完整 worktree/资源治理未完成。见 [实现进度](21-implementation-status.md)。
