...
本计划读取的仓库基线为 `b4efad93076223475f9410e0b1e64708aaf40fcc`。文档目录中没有应用实现，因此本次是从产品目标拆工程任务，不是对已运行软件出具改造报告。

## 2. 工程决策草案

| ID | 建议 | 当前状态与影响 |
| --- | --- | --- |
| DD-01 | TypeScript monorepo；React/Vite 网页；Fastify 服务；Node 执行器；PostgreSQL | 建议基线，01-01 记录团队实际采用值；不宣称获批准或锁定最新版本 |
| DD-02 | 模块化单体、SQL 事务+outbox；浏览器 HTTP/SSE，节点主动 WSS | 建议基线；优先降低多套调度与状态同步成本 |
| DD-03 | 身份复用成熟认证组件，经 IdentityPort 隔离 | 组件待选，不自行实现密码学；可先开发身份契约和模拟页面 |
| DD-04 | Claude SDK 交互路径优先，CLI 为可受限兼容路径；Codex App Server 本地 stdio | 账号方式和已安装工具影响实际选择；08/09 分别记录真实能力 |
...
| DD-08 | 已有 UI 多图作为参考，按 tokens 和真实页面组件实现 | 不把图中示例数据、模型名和按钮自动当作已批准接口 |
| DD-09 | 具体语言/组件版本、首批 OS、Git 平台、部署容量及模型付费主体由团队采用时记录 | 不阻碍先写契约和页面，不编造公司基础设施与人力 |

以上技术建议不是新的强制审批表。工程实施者可在有依据时调整，但需同步契约和相关计划，不能让不同包同时采用互不兼容的默认值。
...
产品行为变化先同步 product 01/03/05；数据和接口变更同步 development 01/18；页面变更同步 02/10/14；新增工作项在所属计划和 19 保持同一 ID。已有 102 项的状态由实际 issue/PR 记录，本次统一待开发。
