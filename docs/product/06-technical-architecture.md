...
> 规划基线 v1.1 · 2026-09-25 · 架构建议，尚无部署与运行实现。  
...
网页可部署在允许的云资源或自管机器，不以成熟内网、VPN、统一身份、Kubernetes 或现成需求系统为前提。对外访问需要 HTTPS 与用户身份验证，公网访问不等于公开任务数据。
...
建议模块化单体控制服务、独立执行器、网页前端与关系型数据库；无需初始引入复杂微服务或全量事件溯源。持久化派发可以通过业务表与可靠后台任务实现。

前端可评估 React／TypeScript，控制服务可评估 TypeScript／Node，数据库可评估 PostgreSQL。执行器可根据团队技能与操作系统选择 TypeScript、Go 或 Rust；本规划不锁定尚未决定的语言和版本。

建议代码边界为 web、control、runner、contracts、adapters、ui。即使共用语言，也需要运行时协议校验和版本兼容，不只依赖编译期类型。

## 4. 单一任务模型

Web、管理概览、AI 执行与成果使用同一 Task 标识。不能再建立一个“AI 工作区任务库”与团队任务同步。
...
文档当前只是设计基线，没有运行数据库或需要执行的数据迁移。不要为兼容未实现的 v1.0 再建设一套旧验收模型。
