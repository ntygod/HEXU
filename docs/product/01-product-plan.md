...
网页提供统一入口，服务可部署于允许的云或自管环境。本地执行器主动连接服务；远程节点提供另一种运行方式，不要求先有 VPN、企业内网或 Kubernetes。
...
默认专业、明亮、克制，提供深色开发工作区。首页突出实际工作和成果，任务页突出继续、协助与并行，成果页突出预览和反馈。高级设置按需展开，常用动作直接可见。

不做员工监控系统、虚拟公司组织图、通用办公套件、完整 IDE、基础模型或代码托管平台；不把重型流程设计器作为基本能力。

当前 UI 概念图只作风格探索，不能替代页面设计。首次欢迎页可以使用品牌展示，日常工作台不以山景、机器人墙或无依据的进度数字作为主内容。
