...
async function start(page: Page) {
  const task = await (await page.request.post('/api/v1/spaces/space-demo/tasks', {
    headers: headers(), data: { title: '持久化接续 · 协议替身测试' },
  })).json();
  ...
  const response = await page.request.post(`/api/v1/tasks/${task.id}/runs`, {
    headers: headers(), data: {
      provider: 'native', requestedTool: 'claude-code', prompt: 'FIXTURE_HANG',
      workingCopyId: native.workspaces[0].id, mode: 'edit', confirmExecution: true,
...
async function cleanup(page: Page, id: string) {
  ...
  for (const op of operations.items ?? []) if (['waiting_for_stop', 'preparing'].includes(op.state))
    await page.request.post(`/api/v1/operations/${op.id}/cancel`, { headers: headers(), data: { expectedRevision: op.revision } });
  const detail = await (await page.request.get(`/api/v1/tasks/${id}`)).json();
  for (const run of detail.runs) if (!['succeeded', 'failed', 'cancelled'].includes(run.state))
  ...
  await expect.poll(async () => {
    ...
    return current.runs.every((r: { state: string }) => ['succeeded', 'failed', 'cancelled'].includes(r.state));
  }).toBe(true);
}
...
test('在运行中的任务直接安排停止后跨工具继续，并保留接续记录', async ({ page }) => {
  ...
  try {
    ...
    await expect(page.locator('.message-content').filter({ hasText: /Codex fixture result \[REDACTED\]/ })).toBeVisible();
    const detail = await (await page.request.get(`/api/v1/tasks/${id}`)).json();
...
test('等待时新增人工要求会暂停接续，原因和原要求在刷新后保留', async ({ page }) => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '原执行结束后继续', exact: true }).click();
    await expect(page.locator('.continuation-status-title strong')).toHaveText('等待原执行结束');
    await page.getByLabel('任务评论', { exact: true }).fill('范围发生变化，先不要自动发送给另一个工具');
    ...
    await page.screenshot({ path: 'artifacts/15-continuation-needs-attention.png', fullPage: true });
    const detail = await (await page.request.get(`/api/v1/tasks/${id}`)).json();
    expect(detail.runs).toHaveLength(1);
  } finally { await cleanup(page, id); }
});
